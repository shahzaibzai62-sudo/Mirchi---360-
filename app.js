...
async function saveAllMenuToFirebase() {
  try {
    const batch = db.batch();
    menuItems.forEach((item, idx) => {
      const ref = db.collection('menu').doc();
      batch.set(ref, { ...item, order: idx });
    });
    await batch.commit();
  } catch (e) { console.error('Firebase menu seed error:', e); }
}

// Save single menu item (add or update)
async function saveMenuItemToFirebase(item, editId) {
  try {
    if (editId) {
      await db.collection('menu').doc(editId).set(item, { merge: true });
      return editId;
    } else {
      const ref = await db.collection('menu').add({ ...item, order: menuItems.length });
      return ref.id;
    }
  } catch (e) { console.error('Firebase save item error:', e); }
  return null;
}

// Delete menu item from Firestore
async function deleteMenuItemFromFirebase(id) {
...
async function saveBookingToFirebase(booking) {
  try {
    const ref = await db.collection('bookings').add({ ...booking, timestamp: firebase.firestore.FieldValue.serverTimestamp() });
    return ref.id;
  ...
  function animate() {
    ...
    for (let i = 0; i < particles.length; i++) {
      for (let j = i + 1; j < particles.length; j++) {
        ...
        if (dist < 80) {  // reduced from 100 for mobile performance
          ctx.save();
...
async function submitBooking() {
  ...
  const bookingId = await saveBookingToFirebase(booking);
  bookings.unshift({ _id: bookingId, ...booking });
  ...
  updateAdminStats();
  if (submitBtn) { submitBtn.disabled = false; submitBtn.textContent = 'Confirm Reservation via WhatsApp'; }
...
firebase.auth().onAuthStateChanged(user => {
  if (user) {
    ...
    // Only render if data already loaded (initAll may still be running on first load)
    if (menuItems.length > 0 || bookings.length >= 0) {
      renderAdminMenuTable();
      renderAdminBookingsTable();
      updateAdminStats();
    }
...
async function adminLogin() {
  ...
  const btn   = document.querySelector('#adminLogin .btn-primary') || document.querySelector('#adminLogin button');

  if (!email || !pass) { err.style.display = 'block'; err.textContent = '⚠️ Email aur password daalen'; return; }
  ...
  try {
    ...
    err.textContent = e.code === 'auth/invalid-credential' || e.code === 'auth/wrong-password' || e.code === 'auth/user-not-found'
      ...
      ? '⚠️ Zyada attempts — thodi der baad try karein'
      : '❌ Login fail hua, dobara try karein';
...
function renderAdminMenuTable() {
  ...
  tbody.innerHTML = menuItems.map((item, idx) => {
    ...
    <tr>
      ...
      <td><strong>${sanitize(item.name)}</strong><br><small style="color:var(--text-muted)">${sanitize(item.desc)}</small></td>
      <td>${item.category}</td>
...
async function clearBookings() {
  if (confirm('Clear all reservations? This cannot be undone.')) {
    ...
    const batch = db.batch();
    bookings.forEach(b => {
      if (b._id) batch.delete(db.collection('bookings').doc(b._id));
    });
    await batch.commit();
    bookings = [];
    renderAdminBookingsTable();
...
async function saveItem() {
  ...
  if (editIdx === -1) {
    // New item — save to Firebase
    const newId = await saveMenuItemToFirebase(item);
    menuItems.push({ _id: newId, ...item });
  } else {
    // Update existing item
    const existingId = menuItems[editIdx]._id;
    await saveMenuItemToFirebase({ ...item, order: editIdx }, existingId);
    menuItems[editIdx] = { _id: existingId, ...item };
  }
...
async function changePassword() {
  ...
  if (!oldP || !newP) { msg.style.color = '#ff6b6b'; msg.textContent = '❌ Sab fields bharein'; return; }
  if (newP.length < 6) { msg.style.color = '#ff6b6b'; msg.textContent = '❌ Naya password kam az kam 6 characters ka hona chahiye'; return; }
  if (newP !== confP) { msg.style.color = '#ff6b6b'; msg.textContent = '❌ Passwords match nahi kar rahe'; return; }

  const user = firebase.auth().currentUser;
  if (!user) { msg.style.color = '#ff6b6b'; msg.textContent = '❌ Pehle login karein'; return; }
  ...
  try {
    ...
    msg.textContent = '✅ Password successfully update ho gaya';
    document.getElementById('oldPass').value = '';
    document.getElementById('newPass').value = '';
    document.getElementById('confirmPass').value = '';
  } catch (e) {
    msg.style.color = '#ff6b6b';
    msg.textContent = e.code === 'auth/wrong-password' || e.code === 'auth/invalid-credential'
      ? '❌ Purana password galat hai'
      : '❌ Password update nahi hua — dobara try karein';
  }
}
...
async function sendAssistantMsg() {
  ...
  // Keep history max 20 messages to avoid large API payloads
